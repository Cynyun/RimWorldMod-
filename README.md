...
直接运行编译后的可执行文件 `RimWorldModManager.exe`，或使用 Visual Studio 2022+ 打开项目进行开发调试。
...
- **导入到游戏**：将 Workshop Mod 复制到游戏目录（自动检测并处理冲突）
...
│   ├── FileConflictDialog.xaml/cs  # 文件冲突对话框（单冲突处理）
│   └── BatchImportOptionsDialog.xaml/cs # 批量导入选项对话框
...
│   ├── ModImportService.cs        # Mod 导入服务（含冲突检测）
...
导入 Mod 时，程序会先检测所有可能的冲突，然后根据冲突数量显示不同的处理界面：

1. **无冲突**：直接导入，无需任何弹窗
2. **单冲突**：显示简洁对话框，提供三个选项：
   - 替换现有文件
   - 保留两者（自动重命名新文件）
   - 跳过此文件
3. **多冲突**：显示冲突数量，并提供批量处理选项：
   - 逐个询问（对每个冲突逐一询问）
   - 全部替换
   - 全部保留
   - 全部跳过
...
`ModImportService.DetectConflicts()` 方法会预扫描所有选中的 Mod，返回冲突列表供 UI 层决定显示哪个对话框。
...
- **冲突处理**：导入多个 Mod 时，建议使用"全部保留"选项避免意外覆盖
